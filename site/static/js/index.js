...
angular.module('kanjiApp', ['ngAnimate', 'ui.router', 'ui.bootstrap-slider', 'dndLists', 'timer']) // [''] contains dependencies.
    ...
    .config(['$animateProvider', '$stateProvider', '$urlRouterProvider', '$sceProvider', '$httpProvider', 
        function($animateProvider, $stateProvider, $urlRouterProvider, $sceProvider, $httpProvider){
...
//		$httpProvider.defaults.headers.common['Authorization'] = 'Basic '+ btoa('thesis:deadline');
            ...
            .state({
                ...
                controller: ["$scope", "$http", function(sc, $http) {
                    ...
                    //     $.ajax({
                    //             url        : "http://user:password@82.37.79.52:8080/generator",
                    //             dataType   : 'json',
...
,
				{
headers: {
'Authorization': 'Basic '+btoa('thesisSpring:deadlineSpring')
}
}
                            )
            ...
            .state({
				...
				password: 'deadline',
                    ...
                    sc.initI = function(jsonParsedresponse){
                        if(sc.stage === 'quizBPending' || sc.stage === 'finished'){
                            sc.x = jsonParsedresponse.list;
                            sc.control = false;
                        }
                        if(sc.stage === 'quizBPendingControl' || sc.stage.indexOf('quizAPending') == 0){
                            sc.x = {};
                            sc.control = true;
                        }
                    ...
                    if(sc.uid != null){
                        sc.readI(sc.uid, '.status'); // this sets sc.stage
                        sc.readI(sc.uid, '.json');
                    }
                }]
            })
            ...
            .state({
				...
				password: 'deadline',
                    ...
                    sc.init = function(jsonParsedresponse){
                        ...
                        if(sc.stage === 'quizAPendingControl' || sc.stage === 'quizBPendingControl') sc.formData.group = 'control';

                        if(sc.stage.indexOf('quizAPending') == 0)  sc.rw = jsonParsedresponse.quizA;
                        else if(sc.stage.indexOf('quizBPending') == 0) sc.rw = jsonParsedresponse.quizB;
                        ...
                        if(sc.stage.indexOf('quizBPending') == 0) sc.feedbackPending = true;
                        // console.log(sc.rw);
				...
				password: 'deadline',
                    ...
                    sc.reportScores = function() {
                        ...
                        if(sc.stage === 'quizAPendingControl'){
                            stage = { "stage": 'quizBPendingControl' };
                            st = 'A';
                            sc.postmanQ(sc.uid, '.status', stage, '');
                            report = sc.quizResultsObj(sc.r);
                        }
                        ...
                        if(sc.stage === 'quizBPendingControl'){
                        stage = { "stage": 'finished' };
                        st = 'B';  
                        sc.postmanQ(sc.uid, '.status', stage, '');
                        report = {
